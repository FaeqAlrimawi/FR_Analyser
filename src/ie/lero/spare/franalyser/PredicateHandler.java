...
public class PredicateHandler {
	...
	public String insertPredicatesIntoBigraphFile(String fileName) {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (list.get(i).startsWith("preds")) {
					isPredDefined = true;
					indexPred = i;
				...
				// keep track of the rules keyword in case there are no
				// predefined predicates
				if (list.get(i).startsWith("rules")) {
					indexRules = i;
				}
			}
	...
	public ArrayList<Predicate> getActivityPredicates(String activityName) {
		...
		/*for (Predicate p : predicates) {
			if (activityName.contentEquals(p.getActivityName())) {
				result.add(p);
			}
		}*/
...
/*	public String insertPredicatesIntoBigraphFile2(String fileName) {
	...
	/**
	 * Returns the first activity in the incident. This activity is the one without any previous activities
	 * @return IncidentActivity object representing the initial activity, null if there is not one
	 */
	...
	public LinkedList<GraphPath> getPathsBetweenActivities(IncidentActivity sourceActivity, IncidentActivity destinationActivity) {
...
/*	public void findAllPossiblePaths() {
		...
		LinkedList<HashMap<String, LinkedList<GraphPath>>> paths = getPathsForIncident();
		GraphPath p = new GraphPath();
		GraphPath p2 = new GraphPath();
		HashMap<String, LinkedList<GraphPath>> tmpHash;
		...
		LinkedList<GraphPath> tmpPath  = paths.get(0).get(getInitialActivity().getNextActivities()
				.get(rand.nextInt(getInitialActivity().getNextActivities().size())).getName());
