...
public String convertToJSONStr() {
		...
		int size = paths.size();
		for(Entry<String, Integer> freq : actionsFrequency.entrySet()) {
			double perc = (int)(((double)freq.getValue()/(double)size)*precision)/(precision*1.0);
			str.append("{")
		...
		//get top paths based on the common actions i.e. paths that contain all common actions
		if(topPaths != null) {
		str.append("\"top_paths\": {")
		.append("\"frequency\":").append(percentageFrequency).append(",")
		.append("\"transitions_ids\":").append(topPaths)
		.append("},");
		} else {
			str.append("\"top_paths\": {")
			.append("\"frequency\":").append(0).append(",")
			.append("\"transitions_ids\":[]")
			.append("},");
		}
		...
		if(shortestPaths != null && shortestPaths.size() >0) {
			actionsNum = paths.get(shortestPaths.getFirst()).getStateTransitions().size()-1;
			str.append("\"shortest_transitions\":{")
			.append("\"number_of_actions\":").append(actionsNum).append(",")
			.append("\"transitions_ids\":").append(shortestPaths)
			.append("},");
		} else {
			str.append("\"shortest_transitions\":{")
			.append("\"number_of_actions\":0,")
			.append("\"transitions_ids\":[]")
			.append("},");
		}
		...
		if(longestPaths != null && longestPaths.size() >0) {
			actionsNum = paths.get(longestPaths.getFirst()).getStateTransitions().size()-1;
			str.append("\"longest_transitions\":{")
			.append("\"number_of_actions\":").append(actionsNum).append(",")
			.append("\"transitions_ids\":").append(longestPaths)
			.append("}");	
		} else {
			str.append("\"longest_transitions\":{")
			.append("\"number_of_actions\":0,")
